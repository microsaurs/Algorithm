...
# 조건 : 첫째 줄에 N과 M이 주어진다. N과 M은 8보다 크거나 같고, 50보다 작거나 같은 자연수이다. 둘째 줄부터 N개의 줄에는 보드의 각 행의 상태가 주어진다. B는 검은색이며, W는 흰색이다. 첫째 줄에 지민이가 다시 칠해야 하는 정사각형 개수의 최솟값을 출력한다.
...
for a in range(N-7):
    for b in range(M-7): #8*8으로 자르기 위해 -7
        w_index=0 #흰색으로 시작
        b_index=0 #검은색으로 시작
