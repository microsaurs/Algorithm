...
movie_n = 666
while True:
    if '666' in str(movie_n): # str로 하지 않으면 무조건 1의 자리부터 시작하기 때문에 str로 형변환
        cnt+=1 # movie_n에 666이 포함되어 있으면 카운트 증가
    if cnt == N: # 카운트랑 N이 같을 때
        print(movie_n) # movie_n 출력
        break # while문 종료
    movie_n += 1 # 666이 포함된 수가 나올 때까지 movie_n을 증가